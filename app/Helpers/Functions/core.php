...
function isFromApi()
{
	...
	if (
		...
		&& \Illuminate\Support\Str::contains(\Route::currentRouteAction(), plugin_namespace('api'))
	) {
...
{
	// Check if the '.env' and 'storagebkk/installed' files exist
...
function genEmailContactBtn($post = null, $btnBlock = false, $iconOnly = false)
{
    ...
    if($xfUser) {
        $btnLink2 = '#'.url("forum/index.php?members/{$xfUser->user_id}");
        $btnClass = ' btn-block';
    }
	...
	if($btnLink2){
        if ($iconOnly) {
            ...
            $out .= '<a href="' . $btnLink2 . '">';
            $out .= '<i class="icon-mail-2 tooltipHere" data-original-title="' . t('Send a message') . '"></i>';
            $out .= '</a>';
        }elseif ($btnBlock){
...
function genPhoneNumberBtn($post, $btnBlock = false)
{
	...
	if (config('settings.single.hide_phone_number')) {
		...
		$btnLink = '';
		$btnAttrTooltip = 'data-toggle="tooltip" data-placement="bottom" data-original-title="' . $btnHint . '"';
		$btnClassTooltip = ' tooltipHere';
	...
	if (!auth()->check()) {
		if (config('settings.single.guests_can_contact_ads_authors') != '1') {
			$btnAttrModal = 'data-toggle="modal"';
	...
	if (config('settings.single.show_security_tips') == '1') {
		...
		*/
		$btnAttrTooltip = 'rel="tooltip" data-placement="bottom" data-original-title="' . $btnHint . '"';
		$btnClassTooltip = 'tooltipHere';
		$btnAttrModal = 'data-toggle="modal"';
	...
	if ($enableWhatsAppBtn) {
		$waBtnAttr = 'data-toggle="tooltip" data-placement="bottom" data-original-title="' . t('chat_on_whatsapp') . '"';
		$waBtnClass = $waBtnClass . ' tooltipHere';
 ...
 * @return string
 */
function genDiscussionBtn($post, $btnBlock = false)
{
	$out = '';
	$xfUser = \XF::em()->findOne('XF:User',['email','=',$post->User->email]);
	if($xfUser){
