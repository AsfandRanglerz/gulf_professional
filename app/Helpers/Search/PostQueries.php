...
class PostQueries
{
	...
	public function __construct($preSearch = [])
	{
		...
		if(request('country_search')){
			...
			$this->posts->verified()->unarchived();
		}
	...
	public function fetch()
	{
		...
		// dd($this->posts->get());
		if(request('country_search') && request('country_search') != '')
		{// Get Results
		$posts = $this->posts->paginate((int)$this->perPage);
		}
		else{
			
			$posts = $this->posts->withoutGlobalScope('App\Models\Scopes\ReviewedScope')->withoutGlobalScope('App\Models\Scopes\VerifiedScope')->withoutGlobalScope('App\Models\Scopes\LocalizedScope')->paginate((int)$this->perPage);
		}
