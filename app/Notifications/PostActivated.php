...
class PostActivated extends Notification implements ShouldQueue
{
	...
	public function __construct(Post $post, $for = 'active')
	{
		$this->post = $post;
		$this->for = $for;
	}
	...
	public function toMail($notifiable)
	{
		...
		if($this->for == 'active'){
			...
			return (new MailMessage)
			->subject(trans('mail.post_updated_title', ['title' => Str::limit($this->post->title, 50)]))
			->greeting(trans('mail.post_updated_content_1',[
				'postUrl' => $postUrl,
				'title'   => $this->post->title,
			]))
			->line(trans('mail.post_updated_content_2', [
				'postUrl' => $postUrl,
				'title'   => $this->post->title,
			]))
			->line(trans('mail.post_updated_content_3', [
				'postUrl' => $postUrl,
				'title'   => $this->post->title,
			]))
			->line(trans('mail.post_updated_content_4', ['appName' => config('app.name')]))
			->salutation(trans('mail.footer_salutation', ['appName' => config('app.name')]));
		}
