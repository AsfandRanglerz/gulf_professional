	...
	public function commonQueries()
	{
		...
		if (config('settings.listing.left_sidebar')) {
			...
			if (config('settings.listing.count_cities_posts')) {
				...
				////Addded by taha  for cities according to search

				if(request('country_search'))
				$cities = City::where('country_code', request('country_search'))->withCount('posts')->take($limit)->orderBy('population', 'DESC')->orderBy('name')->get();
				
				// $cities = Cache::remember($cacheId, $this->cacheExpiration, function () use ($limit) {
				// 	return City::where('country_code', request('country_search'))->withCount('posts')->take($limit)->orderBy('population', 'DESC')->orderBy('name')->get();
				// });
				else
				$cities = Cache::remember($cacheId, $this->cacheExpiration, function () use ($limit) {
				...
				$cities = City::where('country_code', request('country_search'))->take($limit)->orderBy('population', 'DESC')->orderBy('name')->get();
