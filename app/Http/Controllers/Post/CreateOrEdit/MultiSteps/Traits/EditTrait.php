...
trait EditTrait
{
	...
	public function postUpdateForm($postIdOrToken, PostRequest $request)
	{
		...
		$user->save();
		// dd($post);
		$this->xfUserUpdate($post);

		$post->notify(new PostActivated($post,'update'));

		// Redirection
		return redirect($nextStepUrl);
		}

		protected function xfUserUpdate(\App\Models\Post $post)
		{
		$xfUser = \XF::finder('XF:User')
		    ->where('laravel_user_id','=',$post->user_id)
		    ->fetchOne();
		$laravelUser = $post->user;
		$xfUser->username = $post->title;
		$xfUser->employer_name = $post->employer;
		$xfUser->designation = $post->designation;
		$xfUser->save();
		}
