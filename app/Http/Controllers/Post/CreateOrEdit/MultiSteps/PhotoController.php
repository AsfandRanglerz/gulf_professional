...
//        dd($file,$mime,$info,$CURLfile);
        ...
        $file = \XF::app()->request->getFile('pictures', false, false);
        $user = auth()->user();
        $xfUser = \XF::finder('XF:User')
            ->where('laravel_user_id','=',$user->id)
            ->fetchOne();

            $url = url("forum/index.php?api/users")."/".$xfUser->user_id."/avatar";//                $file = $request->input('pictures')[0];
                $file = Storage::path($picture->filename);
              $fields = array('avatar'=>
               new \CURLFILE($file));
