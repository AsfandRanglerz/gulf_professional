...
trait SettingsTrait
{
	...
	public function applyFrontSettings()
	{
		...
		*/
		...
		try {
			$this->og->siteName(config('settings.app.app_name', 'LaraClassified'))->locale($locale)->type('website')->url(rawurldecode(url()->current()));
		} catch (\Exception $e) {
		};
		...
		$pages = Cache::remember('pages.' . config('app.locale') . '.menu', $this->cacheExpiration, function () {
			return Page::trans()->where(function ($query) {
