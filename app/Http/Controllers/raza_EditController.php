...
class EditController extends AccountBaseController
{
	 ...
	 */
	public function updatePhoto($userId, AvatarRequest $request)
	{
		if (isDemo()) {
		...
		if (!empty($file)) {
			...
			$user->photo = $file;
			$avatar = \XF::app()->request->getFile('photo', false, false);
			$avatarService = \XF::app()->service('XF:User\Avatar', \XF::visitor());
			$avatarService->setImageFromUpload($avatar);
			$user->save();
		}
