...
class PostObserver
{
	...
	public function updating(Post $post)
	{
		...
		if (config('settings.mail.confirmation') == 1) {
			// dd($original['verified_email']." ".$original['verified_phone']);
			try {
				...
				// // Post Email address or Phone was verified, and Post was not approved (reviewed)
				// if (($original['verified_email'] == 1 || $original['verified_phone'] == 1) && $original['reviewed'] == 0) {
				// 	// dd('hello4');
				// 	if ($post->verified_email == 1 || $post->verified_phone == 1) {
				// 		$post->notify(new PostReviewed($post));
				// 		// $post->notify(new PostActivated($post,'update'));
