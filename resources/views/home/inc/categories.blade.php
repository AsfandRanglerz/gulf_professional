		...
		<div class="col-xl-12 content-box layout-section">
			<div class="row row-featured row-featured-category">
				<div class="col-xl-12 box-title no-border">
					<div class="inner">
						<h2>
							<span class="title-3">Find <span style="font-weight: bold;">Professionals</span></span>
								...
								<a href="{{ \App\Helpers\UrlGen::category($cat) }}">
									...
									<h6>
										...
										@if (config('settings.listing.count_categories_posts'))
											&nbsp;({{ $countPostsByCat->get($cat->tid)->total ?? 0 }})
										@endif
									</h6>
								</a>
							</div>
												...
												<div class="cat-list">
													<h3 class="cat-title rounded">
														...
														<a href="{{ \App\Helpers\UrlGen::category($iCat) }}">
															{{ $iCat->name }}
															@if (config('settings.listing.count_categories_posts'))
																&nbsp;({{ $countPostsByCat->get($iCat->tid)->total ?? 0 }})
															@endif
														</a>
														...
														@if (isset($subCategories) and $subCategories->has($iCat->tid))
															@foreach ($subCategories->get($iCat->tid) as $iSubCat)
																<li>
																	...
																	@if (config('settings.listing.count_categories_posts'))
																		&nbsp;({{ $countPostsByCat->get($iSubCat->tid)->total ?? 0 }})
																	@endif
																</li>
															@endforeach
														@endif
