				...
				<span class="font-weight-bold">
					...
					@else
						<a href="{{ $searchUrlWithoutCat }}">
							<i class="fas fa-reply"></i> ALL PROFESSION CATEGORIES
						</a>
					@endif
				</span>
				</h5>
				...
				<div class="block-content list-filter categories-list">
					<ul class="list-unstyled">
						<li>
							...
							?>
							<a href="{{ $iUrl }}" title="{{ $cat->parent->name }}">
								<span class="title font-weight-bold">{{ $cat->parent->name }}</span>
								@if (config('settings.listing.count_categories_posts'))
									<span class="count">&nbsp;({{ $countPostsByCat->get($cat->parent->tid)->total ?? 0 }})</span>
								@endif
							</a>
					<ul class="list-unstyled">
