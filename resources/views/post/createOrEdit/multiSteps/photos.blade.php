...
if (request()->segment(2) == 'create') {
    ...
    // dd($post);
    $nextStepUrl = url(App\Helpers\UrlGen::postUri($post));
}
?>
@section('content')
{{-- {{dd(url($nextStepUrl))}} --}}
                                    ...
                                    <fieldset>
                                        @if (isset($picturesLimit) and is_numeric($picturesLimit) and $picturesLimit > 0)
                                            ...
                                            <div id="picturesBloc" class="form-group row">
												<label class="col-md-12 control-label{{ $picturesError }}" for="pictures" > Photograph is required for creating a profile </label>
